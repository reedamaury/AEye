    ...
    for iq=1:length(k) % length of laser rows  % baseline shots on grid redundant ops can accelerate obvious
        for iqq=1:length(kaz) % length of laser columns 
            ...
            if(~isempty(temp))
               ...
               hummaQ(:,5) = shot_number; % shot number 
...
for i=1:length(frame)
    ...
    scatter3(xyz_road{i}(:,1),xyz_road{i}(:,2),xyz_road{i}(:,3),10,xyz_road{i}(:,4),'filled'); colorbar; ylim([-10 10]); zlim([-2 5]); view(-90,35); xlabel("X (m)");ylabel("Y (m)");zlabel("Z (m)")
    keyboard
end